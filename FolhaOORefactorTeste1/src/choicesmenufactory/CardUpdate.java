...
public class CardUpdate implements ChoiceOperation{
    ...
    public void execute(ArrayList<Employee> arrayList, MyCalendar date) {
        cardUpdate(arrayList);
    }

    private void cardUpdate(ArrayList<Employee> arrayList) {
        String id;
        int index;
        while(true){
            ...
            if(id.equalsIgnoreCase("quit")) {
                ...
                update(arrayList, id, index);
        }

    }

    private void update(ArrayList<Employee> employees, String id, int index) {
